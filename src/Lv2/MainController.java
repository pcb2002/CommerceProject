...
public class MainController {
    ...
    public MainController(Scanner sc, CommerceSystem commerceSystem, OrderSystem orderSystem,
                          AdminSystem adminSystem, CategoryController categoryController) {
        this.sc = sc;
        ...
        while(true){
            ...
            try {
                ...
                if (mainOption > 0 && mainOption <= categoryCount) {
                    // 💡 1. 커머스 시스템에 쇼핑을 시키고, 결과물(CartItem)을 받아옵니다.
                    CartItem itemToAdd = commerceSystem.handleShopping(mainOption - 1);

                    // 💡 2. 결과물이 있다면(구매 확정을 눌렀다면) 메인 컨트롤러가 오더 시스템에 넣어줍니다.
                    if (itemToAdd != null) {
                        orderSystem.addToCart(itemToAdd.getProduct(), itemToAdd.getQuantity());
                        System.out.println(itemToAdd.getProduct().getProductName() + "이(가) 장바구니에 담겼습니다.\n");
                    }
                }
    ...
    private void printMainMenu() {
        ...
        for (int i = 0; i < categoryController.getCategories().size(); i++) {
            Category c = categoryController.getCategories().get(i);
